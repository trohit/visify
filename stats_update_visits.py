...
def updateUnitStats(recInfo, vitime, username, password, database, host):
    ...
    dictBlob = dict()
    dictBlob['vitime'] = vitime
    dictBlob['recordid'] = recId
    #dictUnitStats[block][flat] = recId
    dictUnitStats[block][flat] = dictBlob
    ...
    while t > 0:
        ...
        if recInfo != False:
            ...
            if 'Other' in block:
                ...
                # first time entries
                updateUnitStats(recInfo, vitime, username, password, database, host)
                hitCount = hitCount + 1
                print "Hit"
            else:
...
def phase3(username, password, database, host):
    ...
    for block in dictUnitStats:
        ...
        for flat in dictFlatsInSomeBlock:
            print("Flat Details\n")
            pprint(dictFlatsInSomeBlock[flat])
            blob = dictFlatsInSomeBlock[flat]
            rid = str(blob['recordid'])
            vitime = str(blob['vitime'])
            print "Unit " + block + ":" + flat + " :" + rid + " vitime:" + vitime
