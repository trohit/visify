...
function disp_visitor_count_blockwise_daily($block_arr, $days=-1)
{
	...
	if ($total_daily_count == 0) {
		...
		$percent_new_visitors = ($new_visitor_count / $total_daily_count) * 100;
	}
	...
	$count_line = "Total Visitor Count:$total_daily_count " . "New visitors:$percent_new_visitors%";
	...
	#print_r($r);
	echo arr2textTable_daily($r, false);
...
$interval = date_diff(date_create(get_from_today(0)), date_create('2015-03-14'));
